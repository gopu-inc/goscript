// lib/shttp/server.gjs
import net      // Module réseau système
import fs       // Système de fichiers

struct Server {
    port: int,
    routes: dict<string, dict<string, fn(ctx: Context): void>>,
    middlewares: [fn(ctx: Context): void],
    static_routes: dict<string, string>,
    socket: int,
    running: bool,
    router: Router
}
